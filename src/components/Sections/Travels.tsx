...
import {SectionId, travel} from '../../data/data';
import {Travel} from '../../data/dataDef';
import useInterval from '../../hooks/useInterval';
import useWindow from '../../hooks/useWindow';
import QuoteIcon from '../Icon/QuoteIcon';
import Section from '../Layout/Section';

const Travels: FC = memo(() => {
  ...
  const next = useCallback(() => {
    if (activeIndex + 1 === travels.length) {
      setTravel(0)();
    } else {
      setTravel(activeIndex + 1)();
    }
  }, [activeIndex, setTravel, travels.length]);
  ...
  // If no travels, don't render the section
  if (!travels.length) {
    return null;
  }

  return (
    <Section noPadding sectionId={SectionId.Travels}>
      <div
          ...
          <div className="flex flex-col items-center gap-y-6 rounded-xl bg-gray-800/60 p-6 shadow-lg">
            <div
              ...
              {travels.map((travel: Travel, index: number) => {
                ...
                return (
                  <Travel isActive={isActive} key={`${travel.name}-${index}`} travel={travel} />
                );
              })}
            </div>
            <div className="flex gap-x-4">
              {[...Array(travels.length)].map((_, index) => {
                const isActive = index === activeIndex;
                return (
                  <button
                    className={classNames(
                    ...
                    onClick={setTravel(index)}></button>
                );
              })}
            </div>
          </div>
...
const Travel: FC<{travel: Travel; isActive: boolean}> = memo(
  ({travel: {text, name, image}, isActive}) => (
    <div
      className={classNames(
